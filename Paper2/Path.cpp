...
namespace paper
{
    ...
    Error Path::arcTo(const Vec2f & _through, const Vec2f & _to)
    {
        ...
        return arcHelper(extent, Segment(this, m_segmentData.count() - 1), vec, _to, *result, nullptr);
    }
    ...
    Error Path::arcTo(const Vec2f & _to, const Vec2f & _radii, Float _rotation, bool _bClockwise, bool _bLarge)
    {
        ...
        return arcHelper(crunch::toDegrees(extent), Segment(this, m_segmentData.count() - 1), vect, _to, center, &matrix);
    }

    Error Path::arcHelper(Float _extentDeg, Segment _segment, const Vec2f & _direction, const Vec2f & _to, const Vec2f & _center, const Mat32f * _transform)
    {
    ...
    static Size smoothIndex(Int64 _idx, Int64 _length, bool _bClosed)
    {
        ...
        return std::min(_idx < 0 && _bClosed
                        ? _idx % _length
    ...
    void Path::smooth(Int64 _from, Int64 _to, Smoothing _type)
    {
        ...
        if (_type == Smoothing::Asymmetric || _type == Smoothing::Continuous)
        {
            ...
            {
                knots[i] = m_segmentData[(j < 0 ? j + m_segmentData.count() : j) % m_segmentData.count()].position;
            }
    ...
    void Path::insertSegments(Size _index, const SegmentData * _segments, Size _count)
    {
        ...
        {
            auto sit = m_segmentData.insert(m_segmentData.begin() + _index, _segments, _segments + _count);
    ...
    {
        return insertSegment(m_segmentData.count(), SegmentData{_handleIn, _pos, _handleOut});
    }
    ...
    namespace detail
    {
        ...
        static void mergeStrokeCap(Rect & _rect, StrokeCap _cap, const SegmentData & _a, const SegmentData & _b, bool _bStart,
                                   const Vec2f & _strokePadding, const Mat32f & _strokeMat, const Mat32f * _transform)
        {
        ...
        static void mergeStrokeJoin(Rect & _rect, StrokeJoin _join, Float _miterLimit,
                                    ...
                                    const Mat32f & _strokeMat, const Mat32f * _transform)
        {
            switch (_join)
            {
    ...
    Maybe<Rect> Path::computeStrokeBounds(const Mat32f * _transform) const
    {
        ...
        for (Size i = 1; i < m_segmentData.count(); ++i)
        {
            detail::mergeStrokeJoin(*result, join, ml, strokeSegs[i - 1], strokeSegs[i], strokeSegs[i + 1], sp, smat, _transform);
        }

        if (isClosed())
        {
            //closing join
            detail::mergeStrokeJoin(*result, join, ml, strokeSegs[strokeSegs.count() - 1],
                                    strokeSegs[0], strokeSegs[1], sp, smat, _transform);
        }
        else
        {
            //caps
            detail::mergeStrokeCap(*result, cap, strokeSegs[0], strokeSegs[1], true, sp, smat, _transform);
            detail::mergeStrokeCap(*result, cap, strokeSegs[strokeSegs.count() - 2],
