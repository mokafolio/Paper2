...
namespace detail
{
void capSquare(const Vec2f & _position,
...
void capOrJoinRound(stick::DynamicArray<Vec2f> & _outPositions,
                    ...
                    Float _theta)
{
    ...
    for (stick::Size i = 0; i <= circleSubdivisionCount; ++i)
    {
        ...
        _outPositions[i] =
            _position + Vec2f(dir.x * cosa - dir.y * sina, dir.x * sina + dir.y * cosa);
    }
}
...
Vec2f joinMiter(const Vec2f & _position,
                ...
                Float & _outMiterLen)
{
    ...
    if (cross == 0)
    {
        return _position; // due to the miter limit this return should not be used as it should
                          // switch to bevel
    }
