    ...
    namespace detail
    {
        ...
        void PathFitter::fit()
        {
            if (m_positions.count() > 0)
            {
                ...
                // {
                //     segs.append(makeUnique<Segment>(doc.allocator(), m_path, m_newSegments[i].position, m_newSegments[i].handleIn, m_newSegments[i].handleOut, segs.count()));
                // }
        ...
        Bezier PathFitter::generateBezier(Size _first, Size _last,
        ...
        bool PathFitter::reparameterize(Size _first, Size _last,
                                        DynamicArray<Float64> & _u, const Bezier & _curve)
        {
        ...
        void PathFitter::chordLengthParameterize(Size _first, Size _last,
                DynamicArray<Float64> & _outResult)
        {
            ...
            {
                _outResult[i - _first] = _outResult[i - _first - 1] + crunch::distance(m_positions[i], m_positions[i - 1]);
            }
            for (Size i = 1; i <= size; i++)
                _outResult[i] /= _outResult[size];
        }

        PathFitter::MaxError PathFitter::findMaxError(Size _first, Size _last,
                const Bezier & _curve, const DynamicArray<Float64> & _u)
        {
