...
class PathFitter
{
  public:
    ...
    void addCurve(const Vec2f & _pointOne,
                  const Vec2f & _handleOne,
                  const Vec2f & _handleTwo,
                  const Vec2f & _pointTwo);

    Bezier generateBezier(Size _first,
                          Size _last,
    ...
    bool reparameterize(Size _first,
                        Size _last,
                        stick::DynamicArray<stick::Float64> & _u,
                        const Bezier & _curve);
    ...
    void chordLengthParameterize(Size _first,
                                 Size _last,
                                 stick::DynamicArray<stick::Float64> & _outResult);

    MaxError findMaxError(Size _first,
                          Size _last,
                          const Bezier & _curve,
                          const stick::DynamicArray<stick::Float64> & _u);
