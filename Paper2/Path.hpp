...
namespace paper
{
    ...
    public:
        ...
        Error arcTo(const Vec2f & _to, const Vec2f & _radii, Float _rotation, bool _bClockwise, bool _bLarge);
    ...
    private:
        ...
        Error arcHelper(Float _extentDeg, Segment _segment, const Vec2f & _direction, const Vec2f & _to,
                        const Vec2f & _center, const Mat32f * _transform);
    ...
    template<class PT>
    SegmentT<PT>::SegmentT(PT * _path, Size _idx) :
        m_path(_path),
        m_index(_idx)
    {
    ...
    template<class PT>
    CurveT<PT>::CurveT(PT * _path, Size _index) :
        m_path(_path),
        m_index(_index)
    {
    ...
    template<class PT>
    template<class OPT>
    CurveT<PT>::CurveT(const CurveT<OPT> & _other) :
        m_path(_other.m_path),
        m_index(_other.m_index)
    {
