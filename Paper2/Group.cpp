    ...
    Maybe<Rect> Group::computeBounds(const Mat32f * _transform, BoundsType _type) const
    {
        if (m_bIsClipped)
        {
            if (m_children.count())
            {
                ...
                // Maybe<Rect> maskBounds = m_children.first()->computeBounds(_transform ? &tmp : nullptr, _type);
                // if (maskBounds)
                // {
                //     Maybe<Rect> childBounds = mergeWithChildrenBounds(Maybe<Rect>(), _transform, _type, true);
                //     if (childBounds)
                //     {
                //         printf("RETURNING DA INTERSECTED BOUNDS %f %f %f %f\n", childBounds->min().x, childBounds->min().y, maskBounds->min().x, maskBounds->min().y);
                //         return Rect(crunch::max(childBounds->min(), maskBounds->min()),
