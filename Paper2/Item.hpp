...
namespace paper
{
    ...
    protected:
        ...
        stick::Maybe<Rect> mergeWithChildrenBounds(
            const stick::Maybe<Rect> & _bounds,
        ...
        void applyTransformToChildren(const Mat32f & _transform);
        ...
        //overwritten by the actual Item types to compute the bounds. _transform will only be passed along
        //if it is different from the items transform (i.e. for Symbols). This seesm a little stupid but
        //allows to use the existing, cached matrix stack for the default case.
        virtual stick::Maybe<Rect> computeBounds(const Mat32f * _transform, BoundsType _type) const;

        static Rect noBounds();


        Decomposed & decomposeIfNeeded(stick::Maybe<Decomposed> & _dec, const Mat32f & _transform) const;
